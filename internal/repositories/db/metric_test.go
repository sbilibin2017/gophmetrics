...
func TestMetricWriteRepository_SaveAndGet(t *testing.T) {
	...
	assert.WithinDuration(t, time.Now(), gotMetric.CreatedAt, time.Second*5)
	assert.WithinDuration(t, time.Now(), gotMetric.UpdatedAt, time.Second*5)

	oldUpdatedAt := gotMetric.UpdatedAt
	...
	assert.True(t, gotMetric.UpdatedAt.After(oldUpdatedAt), "expected updated_at to be later after update")
}
...
func TestMetricReadRepository_List(t *testing.T) {
	...
	for _, m := range listedMetrics {
		...
		// Timestamp checks
		assert.WithinDuration(t, time.Now(), m.CreatedAt, time.Second*5)
		assert.WithinDuration(t, time.Now(), m.UpdatedAt, time.Second*5)
	}
